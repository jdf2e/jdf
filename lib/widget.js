...
widget.preview = function(name) {
    ...
    let localServerPort = jdf.config.localServerPort;
    findPort(localServerPort, (data)  => {
        if (!data) {
            logger.info(`Port ${localServerPort} is tack up`);
            localServerPort += 1000;
            jdf.config.localServerPort = localServerPort;
        }

        server.init(target + '/', jdf.config.localServerPort);
        openner.open(`http://localhost:${jdf.config.localServerPort}/${name}.html`);
        logger.info('open your browser to see it');
        ...
        if (start == questions.length) {
            ...
            rl.question(q + ': ', function(answer) {
                if (answer != 'y' && answer != 'n') {
                    logger.info("answer choice 'y' or 'n'");
