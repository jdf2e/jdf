...
compressScheduler.init = (srcDir, distDir) => {
    ...
    if (tasks.length <= jdf.config.compressThreadCrisis) {
        logger.info(`with a few tasks (less than ${jdf.config.compressThreadCrisis}),  => 1 cpu using`);
        return compressScheduler.singleProcess(srcDir, distDir, tasks);
    }

    logger.info(`too many tasks (greater than ${jdf.config.compressThreadCrisis}),  => ${cpus.length} cpu using`);
