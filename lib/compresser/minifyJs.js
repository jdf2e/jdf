...
module.exports = {
    ...
    ecmaSourceMinify(source) {
        var result = UglifyES.minify(source);
        if (result.error) {
            throw result.error;
        } else {
            return result.code;
     ...
     * 压缩整个 js 文件 并生成一个新文件
     */
    jsFileMinify: function (srcPath, distPath, config) {
        let jsString = f.read(srcPath);

        try {
            jsString = this.jsSourceMinify(jsString);
        } catch (e) {
            jsString = this.ecmaSourceMinify(jsString);
        }
        if (config.output.hasBanner) {
            jsString = urlReplace.setPrefixBanner(distPath) + jsString;
        }
        f.write(distPath, jsString);
    }
};
