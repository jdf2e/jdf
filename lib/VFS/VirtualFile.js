...
class VFile {
    constructor(oPath, oContent, tPath, tContent) {
        if (oPath && !path.isAbsolute(oPath)) {
            logger.error(`new VFile()--originPath must be absolute.${oPath}`);
            throw new Error(`new VFile()--originPath must be absolute.${oPath}`);
        }
        if (!fs.existsSync(oPath)) {
            logger.error(`new VFile()--file not exsit.${oPath}`);
            throw new Error(`new VFile()--file not exsit.${oPath}`);
        }
