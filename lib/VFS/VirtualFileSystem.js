 ...
 *  readFilesInOriginDirSync(dirname): 从this.originDir中读取文件，也可以自己设置读取文件的目录入口
...
class VFS {
    ...
    setOriginDir(originDir) {
        this.originDir = path.resolve(path.normalize(originDir));
    }
    ...
    setTargetDir(targetDir) {
        ...
        this.targetDir = path.resolve(path.normalize(targetDir));
    }
    ...
    readFilesInOriginDirSync(srcPath) {
        ...
        files.forEach(file => {
            // 不符合的后缀过滤掉
            if (!fileType.isPCExtname(file)) {
                return;
            }

            let fullpath = path.normalize(path.resolve(this.originDir, file));
    ...
    updateFile(filepath, content) {
        let hasIn = this.fileList.some((vfile, index) => {
            if(filepath == vfile.originPath){
                this.fileList[index] = new VFile(filepath, content, this.genTargetPath(filepath), content);
                return true;
            }
        });
    ...
    deleteDir(dirname, whichPath) {
        ...
        // 加path.sep是为了防止匹配a/b/c时匹配到a/b/cc
        let reg = new RegExp(escapeStringRegexp(dirname + path.sep));
        logger.verbose(`deleteDir dir's RegExp: ${reg}`);
        this.fileList = this.fileList.filter(vfile => {
            return !reg.test(vfile[whichPath]);
        });
    }
