...
class VFS {
    constructor() {
        this.fileList = [];
        this.ignore = ['test', '..', '.git', '.svn', 'node_modules', 'Thumbs', 'DS_Store', '.db'];
        this.ignoreFile = ['/.*', 'config.json'];
        this.originDir = '';
        this.targetDir = '';
    }
    ...
    getIgnoreFileGlob() {
        return this.ignoreFile.map(item => {
            return item;
        });
    }

    addIgnoreDir(dirname) {
        this.ignore = this.ignore.concat(dirname);
    }
    ...
    // 仅保存源文件的绝对路径，不对它进行读取
    readFilesInOriginDir() {
        return new Promise((resolve, reject) => {
            let patterns = this.getFilePatterns();
            let options =  {
                cwd: this.originDir,
                ignore: this.getIgnoreGlob().concat(this.getIgnoreFileGlob()),
                dot: true,
                matchBase: true,
                nocase: true,
                nodir: true
    ...
    addFile(filepath, content) {
        ...
        if (notIn) {
            this.fileList.push(new VFile(filepath, content, this.genTargetPath(filepath)));
        } else {
    ...
    queryFile(filepath, whichPath) {
        ...
        if (typeof idx === 'number') {
            ...
            logger.debug(`query file -- not found：${filepath}`);
            return undefined;
        }
    }
        ...
        this.fileList.forEach(vfile => {
            ...
            if (vfile.status !== VFile.status.WRITTEN) {
                if (vfile.isTextFile()) {
                    ...
                    done.push(this.copyFile(vfile.originPath, vfile.targetPath));
                }
        ...
        this.fileList.forEach(vfile => {
            ...
            if (!(tPath === vfile.targetPath && vfile.status !== VFile.status.WRITTEN)) {
                if (vfile.isTextFile()) {
                    ...
                    done.push(this.copyFile(vfile.originPath, tPath));
                }
