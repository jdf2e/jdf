 ...
 * @widget解析器
 */
jdf.widgetParser = require('./buildWidget');
 ...
 * 用于使用jdf api时，设置全局配置
 */
jdf.setConfig = function(options = {}){
    // 工程所在目录
    jdf.currentDir = path.normalize(options.projectPath || f.currentDir());
}
...
jdf.output = function (outputList, options) {
    ...
    if(outputType == 'static'){
        fileType.outputIgnore.extname = fileType.outputIgnore.extname.concat(fileType.extname.html);
    }
...
jdf.checkValidDir = function () {
   let isJDFDir = jdf.mergeConfig();
    if (!isJDFDir) {
        logger.error(`${jdf.currentDir} is not a jdf root dir, please check the right path or use "jdf s -o" to run a local server.`);
...
jdf.mergeConfig = function () {
    var userConfigPath = path.join(jdf.currentDir, jdf.config.configFileName);
...
jdf.getProjectPath = function () {
    ...
    if (jdf.config.projectPath != null) {
        ...
        result = path.basename(jdf.currentDir);
    }
    return result;
}
...
jdf.createStandardDir = function (dir, options) {
    ...
    fileArray[2] = path.join(jdf.config.cssDir, 'style.scss');
 ...
 * @将组件安装到本地
 */
jdf.install = function(componentName, options){
    const install = require('./install');

    if(componentName){
        install.download(componentName);
