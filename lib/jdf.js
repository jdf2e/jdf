...
jdf.output = function(outputList, options) {
    ...
    VFS.addIgnoreDir(jdf.outputDirName);
    VFS.setTargetDir(jdf.transferDir);
...
jdf.getTransferDir = function (projectname) {
        let list = ['LOCALAPPDATA', 'HOME', 'APPDATA'];
        let len = list.length;
        let dir;
        for (let i = 0; i < len; i++) {
            if (dir = process.env[list[i]]) {
                break;
            }
        }
        ...
        dir = path.join(dir, '.jdf-temp/project', projectname);
