...
jdf.output = function(dir, options) {
    ...
    let projectdir = f.currentDir();
    let outputdir = path.normalize(path.join(f.currentDir(), jdf.config.outputDirName, jdf.config.projectPath));
    VFS.setOriginDir(projectdir);
    VFS.setTargetDir(outputdir);

    VFS.readFilesInOriginDir().then(function(){
        return Output.init(VFS, {
            type: outputType,
            isDebug: isDebug,
            isPlain: isPlain
        });
