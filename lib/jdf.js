...
jdf.release = function(options, callback, type) {
    ...
    if(options.plain){
        ...
        var bsConf = {};
        bsConf.autoOpenurl = autoOpenurl;
        bsConf.comboDebug = comboDebug;
        bsConf.type = type;
        bsConf.jdfconfig = jdf.config;
        bsConf.serverDir = jdf.bgCurrentDir;
        bsConf.buildDir = jdf.getProjectPath();
        bsConf.currentDir = jdf.currentDir;
        bs.startup(bsConf, function (port) {
...
jdf.output = function(dir, options, callback) {
    ...
    f.del(jdf.bgCurrentDir, function() {
        ...
        if (dir || options.html || options.rjs || options.debug || options.backup || options.path) {
            //custom自定义
            outputType = 'custom';
            outputList = dir;
