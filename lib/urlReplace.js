...
urlReplace.addSourceCdn = function (source, filename, loadType) {
    ...
    if(!$.is.httpLink(filename)){
        /**
         * 当使用require加载文件，并且文件路径是以字母开头的，直接添加cdn返回
         * 当使用seajs.use加载文件，并且文件路径是以“jdf/”或者“felibs/”开头的，直接添加cdn返回
         */
        if ((loadType == 'require' && /^\w/.test(filename)) || (loadType == 'use' && /^(jdf\/|felibs\/)/.test(filename))) {
