    ...
    return VFS.go()
        .then(() => {
            return VFS.travel((vfile, done) => {
                ...
                var linkReplace = config.output.linkReplace,
                cssImagesUrlReplace = config.output.cssImagesUrlReplace,
                scriptReplace = config.output.jsUrlReplace,
                jsUrlReplace = config.output.jsUrlReplace;
                ...
                if ($.is.html(source)) {
                    ...
                    if (scriptReplace) {
                        $$('script').each(function (index, script) {
                            ...
                            var inlineScript = $$(script).html();
    
                            if(!type || type == 'text/javascript'){
                                if(inlineScript){
                                    $$(script).text(jsAstMain(inlineScript, vfile.originPath));
                                }
                            }
                    ...
                    content = $$.html();
                    if(outputType != 'plain' && outputType != 'debug'){
                        logger.verbose(`combo url: ${source}`);
                        content = urlReplace.comboUrlPath(content);
                    }
                }

                if ($.is.js(source) && jsUrlReplace) {
                    logger.verbose(`replace url: ${source}`);
                    content = jsAstMain(content, vfile.originPath);
                }

                if ($.is.css(source) && cssImagesUrlReplace) {
                    logger.verbose(`replace url: ${source}`);
                    content = urlReplace.cssImagesUrlReplace(content, function (url) {
                ...
                function jsAstMain(content, filepath){
                    return jsAst.main(content, {filepath: filepath}, function(nodeObj){
                        var name = nodeObj.name;
                        var src = nodeObj.str;
                        var loadType = nodeObj.loadType;

                        name = urlReplace.addSourceCdn(source, name, loadType);
                        src = urlReplace.addSourceCdn(source, src, loadType);
...
*/
urlReplace.cssImagesUrlReplace = function (content, cb) {
    ...
    if (tempSet.size) {
        ...
        for (var i of tempSet.values()) {
            ...
            if ($.is.imageFile(b) && !$.is.httpLink(b) && b.indexOf('?__base64') == -1) {
                var sReg = new RegExp(`url\\([\\"\\']{0,1}${esr(b)}[\\"\\']{0,1}\\)`, 'gim');
                content = content.replace(sReg, 'url(' + cb(b) + ')');
            }
        };
    };
...
urlReplace.addSourceCdn = function (source, filename, loadType) {
    ...
    //当没有传入filename时，我会认为你是要处理当前文件，所以loadType需要置为false
    if (!filename) {
    ...
    if(!$.is.httpLink(filename)){
        /**
         * 文件路径是以“jdf/ felibs/ seajs/ virtuals/”开头的，直接返回
         * 当使用require或者seajs.use加载文件，并且文件路径是以字母开头的，直接添加cdn返回
         */
        if(/^jdf\//.test(filename) || /^felibs\//.test(filename) || /^seajs\//.test(filename) || /^virtuals\//.test(filename)){
            // jdf,felibs开头的也在buildOutputWidget中使用了
            return '//misc.360buyimg.com/' + filename;
        }
        else if ((loadType == 'require' || loadType == 'use') && /^\w/.test(filename)) {
            return cdn + $.pathJoin('/', filename);
        }
