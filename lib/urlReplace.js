...
urlReplace.addJsDepends = function (source, content) {
     ...
     *  seajs.use(['/a.js', '/b.js'],function(){}) ==>
     *  seajs.use(['projectPath/a.js', 'projectPath/b.js'],function(){})
     *  seajs.use('../widget/common/common.js');
     */
    function useHandle() {
        var hasSeajs = [];
        var arr1 = content.match(/seajs\.use\((.|\S|\s)*?,\s*function/gim);
        var arr2 = content.match(/seajs\.use\((.*?)\)/gim);

        if(arr1 && arr1.length){
            hasSeajs = hasSeajs.concat(arr1);
        }

        if(arr2 && arr2.length){
            hasSeajs = hasSeajs.concat(arr2);
        }
