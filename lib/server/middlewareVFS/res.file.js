...
const configHTML = require('./res.dirView/config.html');
...
fileRes.getFileContent = function (absPath) {
    ...
    if(path.basename(absPath) == 'jdfConfig.html'){
        return configHTML.html(absPath);
    }
    ...
    if (extname === '.html' || extname === '.xhtml' || extname === 'htm') {
        return injector.injectHTML(vfile.targetContent);
    }
