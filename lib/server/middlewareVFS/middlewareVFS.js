...
server.init = function (projectDir) {
    ...
    return function (request, response, next) {
        ...
        if(requestUrl.includes('saveJdfConfig')){
            let configFile = path.join(projectDir, jdf.config.configFileName);
            let cfd = JSON.parse(fs.readFileSync(configFile));
            let cd = requestUrl.split('?')[1].split('=');
            let name = cd[0].split('.');
            let value = cd[1];
            ...
            if(name.length > 1){
                ...
                cfd[name[0]][name[1]] = value;
            }
            else{
                cfd[name[0]] = value;
            }
            fs.writeFileSync(configFile, JSON.stringify(cfd, null, '    '));

            jdfRes = {
                status: 200,
                contentType: 'application/json;charset=utf-8',
                content: JSON.stringify({success: true, msg: "保存成功"})
            }
        }
        else{
