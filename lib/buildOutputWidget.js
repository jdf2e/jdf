...
bow.initGlobalOutputName = function () {
    ...
    try {
        ...
        logger.warn('can\'t find widget dir in your jdf project, skip combine your widget js/css file.')
...
bow.concatOutputContent = function (info, htmlVfile) {
    ...
    if (info.concatTag.js) {
        $$('script[source=widget]').each(function (idx, el) {
            let src = $$(el).attr('src');
            src = src.split('/');
            if (src) {
                jsSet.add(src[src.length - 1].replace('.js', ''));
            }
        })
        
    }
    if (info.concatTag.css) {
        $$('link[source=widget]').each(function (idx, el) {
            let href = $$(el).attr('href');
            href = href.split('/');
            if (href) {
                cssSet.add(href[href.length - 1].replace('.css', ''))
            }
        })
    }
    
    // 下面采用解析widget标签的形式，上面是直接读取页面中source=widget的script,link标签
...
bow.jsPathRelative = function (vfile, targetDir) {
    ...
    return jsAst.main(content, {filepath: vfile.originPath}, function(nodeObj) {
        var str = nodeObj.str,
            fullpathstr;

        /**
         * 以下情况不处理
         * 1、http路径；2、jdf,felibs,virtuals开头；3、绝对路径
         */
        if(!str ||
            $.is.httpLink(str) ||
            /^jdf\//.test(str) ||
