	...
	if(background){
		...
		background.forEach(function(item){
			...
			if(img_url && reg_is_sprite.test(img_url[0]) && !$.is.httpLink(item)){
                ...
                logger.verbose(`sprite file path: ${realFilePath}`);
	...
	if(result.length>0){
		...
		result.forEach(function(item, i){
			if(!item.repeat){
				...
				var backgroundNew = item.item.replace(urlOrigin, urlOriginNew).replace(reg_is_sprite,'').replace(/__rem\d+/, '');
				if(cssSpriteDirection == 'vertical'){
					if(item.rem){
		...
		background.forEach(function(item){
			...
			if(img_url && reg_is_sprite.test(img_url[0]) && !$.is.httpLink(item)){
                ...
                if(f.exists(realFilePath)){
                    ...
                    logger.error(`${res.url} is not exist`);
                }
			}
		});
	}
