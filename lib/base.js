...
var $ = module.exports = {
    ...
    */
    pathJoin: function(){
        return Array.from(arguments).reduce((m, n) => path.join(m, n)).replace(/\\/g, '/');
    },
    /**
     * @去空格
     */
    ...
    /**
     * clean the comments in html, but not the IE conditional comments
     *  see https://msdn.microsoft.com/en-us/library/ms537512(v=vs.85).aspx
     *  standard HTML comment   <!-- Comment content  -->
     *  downlevel-hidden    <!--[if expression]> HTML <![endif]-->
     *  not implemented <del> downlevel-revealed  <![if expression]> HTML <![endif]> </del>
     * @param  {String} htmlstr 
     * @return {String} htmlstr
     */
    cleanComments: function (htmlstr) {
        var commentReg = /<!--[^]*?-->/gm;
        var IEcommentReg = /<!--\[if\s*.*?\]>([^]*?)<!\s*\[endif\]\s*-->/g;
        htmlstr = htmlstr || '';
        var comments = htmlstr.match(commentReg) || [];
        var len = comments.length;
        for (var i = 0; i < len; i++) {
            IEcommentReg.lastIndex = 0;
            if (!IEcommentReg.test(comments[i])) {
                htmlstr = htmlstr.replace(comments[i], '');
            }
        }
        return htmlstr;
    }
