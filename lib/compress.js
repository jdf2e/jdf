...
compress.addJsDepends = function(source) {
    ...
    var requireArr = sourceCode.match(requireReg);

    var dependenceReg = /define\(.*?['|"].*?['|"].*?,\s*\[.*?\],\s*function/m;

    if(requireArr && requireArr.length){
        requireHandle();
    }
    ...
    function requireHandle(){
        for (var i = 0; i < requireArr.length; i++) {
            var temp = requireArr[i].match(/require\((.*?)\)/);
            if (temp) {
                let match = temp[1].replace(/'|"/g, '');

                //不是以http路径并且不以jdf开头的路径
                if(/^jdf/.test(match)){
                    match = cdn + path.join('/', match).replace(/\\/g, '/');
                }else if(!$.is.httpLink(match)){
                    match = compress.addSourceCdn(source, match);
                }

                sourceCode = sourceCode.replace(requireArr[i], 'require("' + match + '")');
            }
        }
    }
    ...
    function dependenceHandle(){
        ...
        if(a){
            ...
            sourceCode = sourceCode.replace(dependenceReg, 'define(' + d[0] + ',[' + d.slice(1).join(',') + '],function');
        }
    }
     ...
     */
     ...
     function defineHandle(){
        ...
        if (sourceCode.match(defineReg)) {
            ...
            sourceCode = sourceCode.replace(defineReg, 'define("' + compress.addSourceCdn(source) + '",[],function');
        }
    }
            ...
            for (let i = 0, j = hasSeajs.length; i < j; i++) {
                ...
                if (t1) {
                    for (var m = 0; m < t1.length; m++) {
                        ...
                        //不是以http路径并且不以jdf开头的路径
                        if(/^jdf/.test(value)){
                            tempObj[key] = cdn + path.join('/', value).replace(/\\/g, '/');
...
compress.addSourceCdn = function(source, filename){
    ...
    if(/^\//.test(filename)){
        ...
        filename = cdn + path.join('/', jdf.getProjectPath(), filename);
    }else{
