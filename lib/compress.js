        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            if (temp) {
                ...
                if(!$.is.httpLink(match) && !/^jdf/.test(match)){
                    ...
                    //如果是以斜杠开头的，直接添加cdn和项目路径
                    if (/^\//.test(match)) {
                        match = cdn + path.join('/', jdf.getProjectPath(), match);
                    }else{
                        let a = sourcedir.replace(path.normalize(outputdir), '');
                        let b = path.normalize(path.join(a, match));
            ...
            if (t1) {
                for (var m = 0; m < t1.length; m++) {
                    ...
                    if(!$.is.httpLink(value) && !/^jdf/.test(value)){
                        ...
                        //如果是以斜杠开头的，直接添加cdn和项目路径
                        if (/^\//.test(value)) {
                            value = cdn + path.join('/', jdf.getProjectPath(), value);
                        }else{
                            let a = sourcedir.replace(path.normalize(outputdir), '');
                            let b = path.normalize(path.join(a, value));
        ...
        for (var i of tempSet.values()) {
            ...
            if (b != 'about:blank' && !$.is.httpLink(b) && !/data:image/.test(b) && b.indexOf('?__base64') == -1) {
                ...
                if(/^\//.test(b)){
                    ...
                    c = cdn + path.join('/', jdf.getProjectPath(), b);
                }else{
                    //得到当前项目的路径和当前文件路径的一个差值
                    var d = sourcedir.replace(path.normalize(outputdir), '');
                    var e = path.normalize(path.join(d, b));
