 ...
 * @returns Promise<any>
 */
compress.init = function (options) {
    var outputType = options.outputType;
    var isDebug = outputType == 'debug' ? true : false;
    ...
    .then(() => {
        return VFS.travel((vfile, done) => {
            ...
            if ($.is.html(source)) {
                ...
                //html minify
                if(config.output.compresshtml && !isDebug){
                ...
                if(config.output.jsUrlReplace){
                    ...
                    content = compress.addJsDepends(source, content);
                    ...
                    $$('script').each(function(index, script){
                        ...
                        if(!$.is.httpLink(src) && src){
                            let cdnSrc = compress.addSourceCdn(source, src);
            ...
            if ($.is.js(source)){
                ...
                //js minify
                if(config.output.compressJs && !isDebug){
            ...
            if ($.is.css(source)) {
                ...
                //css CleanCSS
                if(config.output.compressCss && !isDebug){
 ...
 *
 */
compress.addJsDepends = function (source, content) {
