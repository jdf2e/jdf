...
buildHTML.init = function () {
    ...
    .then(() => {
        return VFS.travel((vfile, done) => {
            ...
            logger.debug(`reset vfile.targetContent to be vfile.originContent: ${vfile.originPath}`);
            ...
            //     dirname: 'widget的绝对路径'
            // }
            logger.debug(`parse widget tag in html, generate widgetInfo object collection.`)
            ...
            logger.debug(`foreach widgetInfo`);
            // 编译单个widget到html中
            widgets.forEach(widgetInfo => {
                logger.debug(`widget: ${widgetInfo.name}`);
                ...
                widgetVfiles.forEach(widgetVfile => {
                    ...
                    if (path.relative(dirname, widgetInfo.dirname)
                        || !oBasename.match(widgetInfo.name)) {
                        // 1、相对路径不为空
                        // 2、文件名不和widget名一致
                        return;
                    }
                    ...
                    if (widgetInfo.buildTag.vm && $.is.vm(wPath)) {
                        logger.debug(`insert vm into HTML: ${widgetVfile.originPath}`);
                        ...
                        logger.debug(`insert smarty into HTML: ${widgetVfile.originPath}`);
                        ...
                        // 所以新代码不维护这个全局变量了
                        logger.debug(`insert <link> tag into HTML: ${widgetVfile.originPath}`);
                        buildHTML.insertCSS(vfile, widgetVfile);
                    } else if (widgetInfo.buildTag.js && $.is.js(wPath)){
                        logger.debug(`insert <script> tag into HTML: ${widgetVfile.originPath}`);
        ...
        logger.error(`err in html %j`, err);
    });
...
buildHTML.insertVM = function (widgetInfo, htmlVfile, vmVfile) {
    ...
    if (!jdf.config.output.vm || !vmVfile.targetContent) {
        replaceStr = '';
    } else {
        try {
...
buildHTML.getWidgetData = function (widgetInfo) {
    ...
    let data = {};
    if (widgetInfo.data) {
        data = JSON.parse(widgetInfo.data);
    }
