...
buildWidget.init = function(inputPath,content,type,callback,param){
	...
	if (result){
		...
		result.forEach(function(resultItem){
			...
			if (widgetType) {
                var buildTag = checkbuildTag(widgetType, buildTag);
            }
			...
			if (dirExists){
				...
				console.log(colors.yellow('jdf warning [jdf.buildWidget] ' +widgetStr +' widget '+ widgetName+ ' does not exist.'));
			}
		});
...
function checkbuildTag(wgtType, tagSet) {
    ...
    if (noSupportType.length > 0) {
        console.warn(colors.yellow("jdf warnning:"), " [widget type - "+ colors.yellow(noSupportType.join(',')) +'] is not approve, please try "tpl,vm,js,css" again ');
    }
